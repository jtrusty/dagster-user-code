...
- Dagster `1.13.1` runtime packages aligned with the current published control-plane release train
- Dagster integration packages aligned on the matching `0.29.1` release line
...
- The image currently targets Python `3.14.4`
- `.python-version` pins local `uv`/Python resolution to the same patch release
...
- It imports definitions from `DAGSTER_PACKAGE_MODULE` (defaults to `project.definitions`)
...
Set `DAGSTER_PACKAGE_MODULE` in your deployment environment to override the module path for a specific project release.
...
```bash
uv sync --locked --extra aws --extra dbt --extra spark
uv run pytest
...
Use Task to build and run the same image shape used in deployment:
...
```bash
IMAGE=dagster-user-code:test ENV_FILE=.env.deploy PORT=4001 task local:deploy
```
...
uv lock --upgrade
uv sync --locked --extra aws --extra dbt --extra spark
uv sync --locked --no-dev --extra aws --extra dbt --extra spark
```
