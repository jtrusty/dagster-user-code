...
DEFAULT_MODULE = "project.definitions"
...
def install_dir() -> Path:
    configured_dir = os.getenv("DAGSTER_PACKAGE_INSTALL_DIR", "").strip()
    return Path(configured_dir or DEFAULT_INSTALL_DIR)
...
def read_text_from_s3(uri: str) -> str:
    ...
    if not text:
        raise RuntimeError(f"S3 object at {uri} was empty")
    return text


def wheel_metadata_from_uri(wheel_uri: str) -> tuple[str | None, str | None]:
    filename = Path(urlparse(wheel_uri).path).name
    try:
        distribution_name, version, _, _ = parse_wheel_filename(filename)
...
def inferred_distribution_from_wheel(wheel_uri: str) -> str | None:
    distribution_name, _ = wheel_metadata_from_uri(wheel_uri)
    return distribution_name


def inferred_version_from_wheel(wheel_uri: str) -> str | None:
    _, version = wheel_metadata_from_uri(wheel_uri)
    return version
